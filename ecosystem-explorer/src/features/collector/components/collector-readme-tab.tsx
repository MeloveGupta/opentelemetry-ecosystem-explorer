...
import { useComponentReadme } from "@/hooks/use-collector-data";

interface CollectorReadmeTabProps {
  name: string;
  markdownHash: string;
}
...
export function CollectorReadmeTab({ name, markdownHash }: CollectorReadmeTabProps): JSX.Element {
  const { t } = useTranslation("collector");
  const { data: markdown, loading, error } = useComponentReadme(name, markdownHash);

  if (loading) {
    return (
      <div className="flex items-center justify-center py-16">
        <Loader2 className="text-primary h-6 w-6 animate-spin" aria-hidden="true" />
        <span className="text-muted-foreground ml-2 text-sm">{t("readme.loading")}</span>
      </div>
    );
  }
  ...
  if (error || !markdown) {
    return (
      <div className="flex flex-col items-center justify-center space-y-2 py-16 text-center">
        ...
        <p className="text-muted-foreground text-sm">{t("readme.error")}</p>
      </div>
    );
  }
